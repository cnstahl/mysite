...
function vote() {
  // synchronous update: every spin is updated in parallel from the old state
  const new_spins = Array.from({ length: L }, () => Array.from({ length: L }, () => [0, 0]));
  for (let y = 0; y < L; y++) {
    for (let x = 0; x < L; x++) {
      for (let layer = 0; layer < 2; layer++) {
        // get four neighbors (within the same layer)
        const sum =
          get_spin(x + 1, y, layer) +
          get_spin(x, y + 1, layer) +
          get_spin(x - 1, y, layer) +
          get_spin(x, y - 1, layer) +
          get_spin(x, y, 1 - layer); // also include the other layer

        // Heat bath dynamics
        const prob_up = 1 / (1 + Math.exp(-2 * (sum - 2.5 - h) / T));
        new_spins[x][y][layer] = Math.random() < prob_up ? 1 : 0;
      }
    }
  }
  spins = new_spins;
...
function loop() {
  if (running) {
    ...
    vote_accum += speed;
    const sweeps = Math.floor(vote_accum);
    vote_accum -= sweeps;
